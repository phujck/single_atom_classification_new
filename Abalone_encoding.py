...
if __name__ == '__main__':
    ...
    AbaloneData=np.load('AbaloneProcessedData.npy')
    ...
    for F in intensities:
        ...
        for omega_2 in [3*omega_0]:
            ...
            print('now evolving')
            # F = 0.04
            spectra=pool.starmap(get_hhg_spectrum,zip(repeat(F),repeat(omega_0),repeat(omega_1),repeat(omega_2),AbaloneData,repeat(False),repeat(False)))
            print(len(spectra))
            # pool.join()
            # spectrum, harmonic = get_hhg_spectrum(F, get_omega=True)
